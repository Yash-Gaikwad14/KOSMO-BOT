...
describe('UnbelievaBoatClient', () => {
  ...
  describe('getUserBalance', () => {
    test('fails if API key is not configured', async () => {
      ...
      const result = await client.getUserBalance('guild-1', 'user-1');
      expect(result.success).toBe(false);
      expect(result.error).toContain('API key is not configured');
    });
    ...
    test('fails if guildId or userId is missing', async () => {
      ...
      const res1 = await client.getUserBalance('', 'user-1');
      expect(res1.success).toBe(false);
      expect(res1.error).toContain('Guild ID and User ID are required');

      const res2 = await client.getUserBalance('guild-1', '');
      expect(res2.success).toBe(false);
      expect(res2.error).toContain('Guild ID and User ID are required');
    });

    test('successfully executes GET request with correct headers and URL', async () => {
      const mockFetch = jest.fn().mockResolvedValue({
        ok: true,
        status: 200,
        json: jest.fn().mockResolvedValue({
      ...
      const result = await client.getUserBalance('guild-123', 'user-456');
    ...
    test('handles HTTP 429 Rate Limiting safely', async () => {
      ...
      const result = await client.getUserBalance('g1', 'u1');
      expect(result.success).toBe(false);
      expect(result.status).toBe(429);
      expect(result.error).toMatch(/rate limit/i);
    });
    ...
    test('handles network fetch rejection / timeout gracefully', async () => {
      const mockFetch = jest.fn().mockRejectedValue(new Error('ECONNRESET: Socket closed'));
  ...
  describe('getGuildLeaderboard', () => {
    test('fails if API key is not configured', async () => {
      ...
      const result = await client.getGuildLeaderboard('guild-1');
      expect(result.success).toBe(false);
      expect(result.error).toContain('API key is not configured');
    });

    test('fails if guildId is missing', async () => {
      const client = new UnbelievaBoatClient({ apiKey: 'secret-key-xyz' });

      const result = await client.getGuildLeaderboard('');
      expect(result.success).toBe(false);
      expect(result.error).toContain('Guild ID is required');
    });

    test('successfully builds correct URL, headers, and query parameters (sort, limit, page)', async () => {
      const mockFetch = jest.fn().mockResolvedValue({
        ok: true,
        status: 200,
        json: jest.fn().mockResolvedValue([
      ...
      const client = new UnbelievaBoatClient({
        apiKey: 'secret-test-key-xyz',
        fetchFn: mockFetch as any,
      });

      const result = await client.getGuildLeaderboard('guild-abc', 'cash', 10, 2);

      expect(result.success).toBe(true);
      expect(result.status).toBe(200);
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(mockFetch).toHaveBeenCalledWith(
        'https://unbelievaboat.com/api/v1/guilds/guild-abc/leaderboard?sort=cash&limit=10&page=2',
        {
          method: 'GET',
          headers: {
            Authorization: 'secret-test-key-xyz',
            Accept: 'application/json',
          },
        }
      );
    ...
    test('successfully handles paginated object response structure', async () => {
      const mockFetch = jest.fn().mockResolvedValue({
        ok: true,
        status: 200,
        json: jest.fn().mockResolvedValue({
      ...
      const result = await client.getGuildLeaderboard('guild-1', 'total', 10, 1);
      expect(result.success).toBe(true);
    ...
    test('handles HTTP 429 Rate Limiting safely', async () => {
      ...
      const result = await client.getGuildLeaderboard('g1');
      expect(result.success).toBe(false);
      expect(result.status).toBe(429);
      expect(result.error).toMatch(/rate limit/i);
    });
    ...
    test('handles HTTP 404 Guild not found', async () => {
      const mockFetch = jest.fn().mockResolvedValue({
        ok: false,
        status: 404,
        json: jest.fn().mockResolvedValue({ message: 'Guild not found' }),
      });
